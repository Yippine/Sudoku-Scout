    ...
    companion object {
        ...
        fun generateSudoku(difficulty: Difficulty): Array<Array<SudokuCell>> {
            ...
            val cellsToRemove = when (difficulty) {
                Difficulty.EASY -> 22      // 超級新手 - 保留大部分數字
                Difficulty.MEDIUM -> 36    // 休閒玩家 - 適中挑戰
                Difficulty.HARD -> 50      // 進階玩家 - 需要運用技巧
                Difficulty.EXPERT -> 60    // 高手 - 複雜邏輯推理
                Difficulty.EVIL -> 68      // 專家 - 極限挑戰
            }

            removeNumbers(grid, cellsToRemove, difficulty)
        ...
        private fun removeNumbers(grid: Array<Array<SudokuCell>>, count: Int, difficulty: Difficulty) {
            ...
            var removed = 0
            val requiresUniqueSolution = when (difficulty) {
                Difficulty.EASY, Difficulty.MEDIUM -> true  // 簡單和中等要求唯一解
                Difficulty.HARD -> true                     // 困難也要求唯一解
                Difficulty.EXPERT, Difficulty.EVIL -> false // 專家和惡魔允許多解
            }
            ...
            for ((row, col) in cells) {
                ...
                var shouldKeepRemoved = true
                if (requiresUniqueSolution) {
                    val workingGrid = copyGrid(grid)
                    shouldKeepRemoved = hasUniqueSolution(workingGrid)
                } else {
                    // 對於高難度，只檢查是否仍有解（不要求唯一）
                    val workingGrid = copyGrid(grid)
                    shouldKeepRemoved = solve(copyGrid(workingGrid))
                }

                if (shouldKeepRemoved) {
                    removed++
                } else {
                    // Restore the number if removing it creates problems
